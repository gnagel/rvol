...
#
# calculates correlations and then a standard deviation from that.
#
class CorrelationSTDreport
  def calculateStdFfromCorrelations
    puts 'loading report'
    date = Time.now
    ...
    tickers.each do |ticker|
      # TODO sort by date
      history = Stockhistorical.all(:symbol => ticker.symbol ,:date.gt=>twentyago)
      arrayPrices = history.collect{|tic| tic.close.to_f }

      tickers2.each do |ticker2|
        history2 = Stockhistorical.all(:symbol => ticker2.symbol ,:date.gt=>twentyago)
        ...
        if ticker.symbol != ticker2.symbol
          ...
          scorrelation.created_at = Time.now
