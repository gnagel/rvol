...
class Test
  ...
  # 1. Positive net income compared to last year (1 point)  -- > changing to quarter!
  ...
  doc.xpath("//tr").each do |tr|
    if (tr.to_s.include?('Net Income/Starting Line'))
      puts latestNet = tr.xpath("td[2]").inner_text
      puts beforeNet = tr.xpath("td[3]").inner_text
      puts BigDecimal.new(latestNet)
      puts #{latestNet - beforeNet}
      break
    end
